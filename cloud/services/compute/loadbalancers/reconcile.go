...
func (s *Service) createOrGetInstanceGroups(ctx context.Context) ([]*compute.InstanceGroup, error) {
	...
	for _, zone := range zones {
		...
		instancegroup, err := s.instancegroups.Get(ctx, cloud.ZonalKey(instancegroupSpec.Name, zone))
		if err != nil {
			...
			if err := s.instancegroups.Insert(ctx, cloud.ZonalKey(instancegroupSpec.Name, zone), instancegroupSpec); err != nil {
...
func (s *Service) createOrGetHealthCheck(ctx context.Context) (*compute.HealthCheck, error) {
	...
	healthcheck, err := s.healthchecks.Get(ctx, cloud.GlobalKey(healthcheckSpec.Name))
	if err != nil {
		if !gcperrors.IsNotFound(err) {
		...
		if err := s.healthchecks.Insert(ctx, cloud.GlobalKey(healthcheckSpec.Name), healthcheckSpec); err != nil {
...
func (s *Service) createOrGetBackendService(ctx context.Context, instancegroups []*compute.InstanceGroup, healthcheck *compute.HealthCheck) (*compute.BackendService, error) {
	...
	backendsvc, err := s.backendservices.Get(ctx, cloud.GlobalKey(backendsvcSpec.Name))
	if err != nil {
		if !gcperrors.IsNotFound(err) {
		...
		if err := s.backendservices.Insert(ctx, cloud.GlobalKey(backendsvcSpec.Name), backendsvcSpec); err != nil {
	...
	if len(backendsvc.Backends) != len(backendsvcSpec.Backends) {
		...
		if err := s.backendservices.Update(ctx, cloud.GlobalKey(backendsvcSpec.Name), backendsvc); err != nil {
...
func (s *Service) createOrGetTargetTCPProxy(ctx context.Context, service *compute.BackendService) (*compute.TargetTcpProxy, error) {
	...
	target, err := s.targettcpproxies.Get(ctx, cloud.GlobalKey(targetSpec.Name))
	if err != nil {
		if !gcperrors.IsNotFound(err) {
		...
		if err := s.targettcpproxies.Insert(ctx, cloud.GlobalKey(targetSpec.Name), targetSpec); err != nil {
...
func (s *Service) createOrGetAddress(ctx context.Context) (*compute.Address, error) {
	...
	addr, err := s.addresses.Get(ctx, cloud.GlobalKey(addrSpec.Name))
	if err != nil {
		if !gcperrors.IsNotFound(err) {
		...
		if err := s.addresses.Insert(ctx, cloud.GlobalKey(addrSpec.Name), addrSpec); err != nil {
...
func (s *Service) deleteInstanceGroups(ctx context.Context) error {
	...
	for zone := range s.scope.Network().APIServerInstanceGroups {
		...
		key := cloud.ZonalKey(spec.Name, zone)
